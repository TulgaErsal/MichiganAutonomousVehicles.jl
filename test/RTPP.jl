...
c = load(open(string(Pkg.dir("MichiganAutonomousVehicles"),"/config/planner/",planner_name,".yaml")))
@testset "cases with (case=>$(case_name)) " for case_name in cases, model in models
  c["vehicle"] = load(open(string(Pkg.dir("MichiganAutonomousVehicles"),"/config/vehicle/",vehicle_name,".yaml")))
